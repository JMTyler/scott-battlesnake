...
const Engine = {
	...
	next : (state, moves, foodSpawns)=>{
		...
		//Create Food
		if(foodSpawns){
			if(foodSpawns[nState.turn]){
				nState.board.food = nState.board.food.concat(foodSpawns[nState.turn]);
			}
		} else {
			if(Math.random() < 0.15){
	...
	play : async (snakes, initState, foodSpawns, cb=()=>{})=>{
		let state = initState;
		let history = [];
		...
		const recur = async ()=>{
			...
			state = Engine.next(state, moves, foodSpawns);
